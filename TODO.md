...
1. **Actually watch the Phase 4 experience run, live, end to end.** Today's session wired real SM-2 progress tracking and depth/word-status replacement probabilities into `ReaderView`/`HomePage`, verified via automated tests (60/60 passing) and a basic app launch (no crash, home screen renders correctly). What's still unverified with real eyes: open a real fixture file, confirm replacements actually render, confirm density visibly ramps as you scroll deeper into the document, confirm tapping a word toggles it and that closing/reopening the app later shows previously-learned words replaced more consistently. This is the actual proof the concept works, not just that the code compiles and unit tests pass against fakes.
