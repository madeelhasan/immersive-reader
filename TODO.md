...
4. **Phase 3 client-side auth/progress-sync** - in progress: `AuthService` (`lib/auth/auth_service.dart` - register/login against the backend, JWT + email persisted via SharedPreferences) and `ProgressSyncService` (`lib/progress/progress_sync_service.dart` - push local `word_progress` rows to `/progress`, pull and upsert the server's copy back) both exist, both TDD-built and independently verified (15 tests total, `flutter analyze` clean). **Not yet wired into any UI** - there's no login/register screen and nothing calls `push`/`pull` yet, so this has no live effect until `HomePage` gets a way to trigger it. This was the one genuinely open item from the SPEC.md review.
...
- ~~Rate limiting on `/auth/login` and `/auth/register`~~ **Done**: `backend/app/rate_limit.py`, an in-memory fixed-window limiter (5 requests/60s per client IP), TDD-delegated to DeepSeek and independently verified (21/21 backend tests pass, including 2 new integration tests that actually trip the 429). Single-process only by design - a real multi-worker/multi-instance deployment still needs a shared store (Redis or similar), noted in the module's own docstring.
...
- A migration path for merging local placeholder-UUID progress (`lib/progress/local_user_id.dart`) into a real account once client-side login exists. `ProgressSyncService.push()` provides a basic version of this for free - it reads whatever `localUserId` it's given, so pointing it at the placeholder ID and pushing under a freshly-obtained account token does move that data onto the account - but there's no UI trigger for it yet (see the critical-path item above), and it's a one-shot push, not real conflict-aware reconciliation (e.g. logging in on a second device would have its own local placeholder history that pull() would silently overwrite, server-wins, no merge)
