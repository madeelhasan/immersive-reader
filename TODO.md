...
2. **B1 vocabulary volume** - in progress: 148 → 227 (three DeepSeek rounds this session, ~50-60 entries each, freely generated with no avoid-list then deduped in Python against the full dataset - the reliable pattern). Yield dropped round over round as topics got picked over (21% → 72% → 33%), so more rounds are needed but each is worth less; still short of the ~400 a B1 reader's share implies. Not strictly blocking the mechanism, but a demo where the level selector visibly runs out of B1 words undercuts the pitch.
